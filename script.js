    ...
    if (isFirstAccess && !usuarioLogado && !savedToken) {
        ...
        if (savedToken && usuarioLogado && usuarios[usuarioLogado.username]) {
            ...
            if (usuarios[usuarioLogado.username].permissao === "Supervisor") {
                ...
                document.getElementById("almoxarifadoSystem").style.display = "block"; // Novo botão
            }else {
                document.getElementById("locateSystem").style.display = "none"; // Adicionado: Esconde o Localizador para Operador
                document.getElementById("almoxarifadoSystem").style.display = "none"; // Novo botão
            }
        } else {
            loginContainer.style.display = "block";
        }
...
function verificarLogin() {
    ...
    if (usuarioLogado && savedToken && usuarios[usuarioLogado.username]) {
        ...
        if (usuarios[usuarioLogado.username].permissao === "Supervisor") {
            document.getElementById("compareSystem").style.display = "block";
            document.getElementById("locateSystem").style.display = "block";
            document.getElementById("almoxarifadoSystem").style.display = "block"; // Adiciona o botão para supervisor
        } else {
            document.getElementById("compareSystem").style.display = "none";
            document.getElementById("locateSystem").style.display = "none";
            document.getElementById("almoxarifadoSystem").style.display = "none"; // Esconde para operador
        }
...
document.getElementById("logoutButton").addEventListener("click", function () {
    ...
    document.getElementById("compareSystem").style.display = "none";
    document.getElementById("locateSystem").style.display = "none";
    document.getElementById("almoxarifadoSystem").style.display = "none"; // Reseta a visibilidade
